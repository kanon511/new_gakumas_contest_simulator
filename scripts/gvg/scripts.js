...
import { ContestData } from '../simulator/data/gvgContestData.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const element_sub1_card_box  = document.getElementById('sub1-character-card-box');
    const element_sub2_card_box  = document.getElementById('sub2-character-card-box');
    const element_sub3_card_box  = document.getElementById('sub3-character-card-box');
    const element_sub4_card_box  = document.getElementById('sub4-character-card-box');
    
    const element_main_cards = DOM_set_characterCards(element_main_card_box, 6, 'main');
    const element_sub1_cards  = DOM_set_characterCards(element_sub1_card_box, 6, 'sub1');
    const element_sub2_cards  = DOM_set_characterCards(element_sub2_card_box, 6, 'sub2');
    const element_sub3_cards  = DOM_set_characterCards(element_sub3_card_box, 6, 'sub3');
    const element_sub4_cards  = DOM_set_characterCards(element_sub4_card_box, 6, 'sub4');

    const element_main_card_box_image = document.getElementById('main-character-cards-box-image');
    const element_sub1_card_box_image  = document.getElementById('sub1-character-cards-box-image');
    const element_sub2_card_box_image  = document.getElementById('sub2-character-cards-box-image');
    const element_sub3_card_box_image  = document.getElementById('sub3-character-cards-box-image');
    const element_sub4_card_box_image  = document.getElementById('sub4-character-cards-box-image');
    
    const element_main_cards_image = DOM_set_characterImageCards(element_main_card_box_image, 6, element_main_cards);
    const element_sub1_cards_image  = DOM_set_characterImageCards(element_sub1_card_box_image, 6, element_sub1_cards);
    const element_sub2_cards_image  = DOM_set_characterImageCards(element_sub2_card_box_image, 6, element_sub2_cards);
    const element_sub3_cards_image  = DOM_set_characterImageCards(element_sub3_card_box_image, 6, element_sub3_cards);
    const element_sub4_cards_image  = DOM_set_characterImageCards(element_sub4_card_box_image, 6, element_sub4_cards);
    ...
    [].concat(element_card_selects, element_card_select_checkboxs).forEach(element=>{
        element.addEventListener('change', (e)=>{
            ...
            const sub1_cards_id = element_sub1_cards.map(element=>Number(element.value)+(element.parentNode.getElementsByClassName('checkbox')[0].checked && element.value % 10 == 0 ? 1 : 0));
            const sub2_cards_id = element_sub2_cards.map(element=>Number(element.value)+(element.parentNode.getElementsByClassName('checkbox')[0].checked && element.value % 10 == 0 ? 1 : 0));
            const sub3_cards_id = element_sub3_cards.map(element=>Number(element.value)+(element.parentNode.getElementsByClassName('checkbox')[0].checked && element.value % 10 == 0 ? 1 : 0));
            const sub4_cards_id = element_sub4_cards.map(element=>Number(element.value)+(element.parentNode.getElementsByClassName('checkbox')[0].checked && element.value % 10 == 0 ? 1 : 0));
            ...
            sub1_cards_id.forEach((item,index)=>{
                if(item==-1||item==0){
                    element_sub1_cards_image[index].src=addImgPath;
                    return;
                }
                element_sub1_cards_image[index].src=imgPath+"cards/card_"+item+".webp"
            })
            sub2_cards_id.forEach((item,index)=>{
                if(item==-1||item==0){
                    element_sub2_cards_image[index].src=addImgPath;
                    return;
                }
                element_sub2_cards_image[index].src=imgPath+"cards/card_"+item+".webp"
            })
            sub3_cards_id.forEach((item,index)=>{
                if(item==-1||item==0){
                    element_sub3_cards_image[index].src=addImgPath;
                    return;
                }
                element_sub3_cards_image[index].src=imgPath+"cards/card_"+item+".webp"
            })
            sub4_cards_id.forEach((item,index)=>{
                if(item==-1||item==0){
                    element_sub4_cards_image[index].src=addImgPath;
                    return;
                }
                element_sub4_cards_image[index].src=imgPath+"cards/card_"+item+".webp"
            })
        })
    });
    ...
    element_sub1_cards[0].parentNode.getElementsByClassName('checkbox')[0].checked = true;
    element_sub2_cards[0].parentNode.getElementsByClassName('checkbox')[0].checked = true;
    element_sub3_cards[0].parentNode.getElementsByClassName('checkbox')[0].checked = true;
    element_sub4_cards[0].parentNode.getElementsByClassName('checkbox')[0].checked = true;
    ...
    function DOM_set_uniqueSkillCard (type, id) {
        ...
        switch (type) {
            case'main':
                element_select = element_main_cards[0];
                break;
            case'sub1':
                element_select = element_sub1_cards[0];
                break;
            case'sub2':
                element_select = element_sub2_cards[0];
                break;
            case'sub3':
                element_select = element_sub3_cards[0];
                break;
            case'sub4':
                element_select = element_sub4_cards[0];
                break;
        }
    ...
    function DOM_set_otherSkillCards (plan) {
        const elements = element_main_cards.slice(1).concat(element_sub1_cards.slice(1)).concat(element_sub2_cards.slice(1)).concat(element_sub3_cards.slice(1)).concat(element_sub4_cards.slice(1));
    ...
    const element_sub1_character_select  = document.getElementById('sub1-character');
    const element_sub2_character_select  = document.getElementById('sub2-character');
    const element_sub3_character_select  = document.getElementById('sub3-character');
    const element_sub4_character_select  = document.getElementById('sub4-character');

    element_main_character_select.addEventListener('change', (e) => {
        const id = Number(e.target.value);
        const pIdol = PIdolData.getById(id);
        DOM_delete_allChildren(element_sub1_character_select);
        DOM_delete_allChildren(element_sub2_character_select);
        DOM_delete_allChildren(element_sub3_character_select);
        DOM_delete_allChildren(element_sub4_character_select);
        if (!pIdol) {
            element_sub1_character_select.disabled = true;
            element_sub2_character_select.disabled = true;
            element_sub3_character_select.disabled = true;
            element_sub4_character_select.disabled = true;
            return;
        }
        element_sub1_character_select.disabled = false;
        element_sub2_character_select.disabled = false;
        element_sub3_character_select.disabled = false;
        element_sub4_character_select.disabled = false;
        const dPIdols = PIdolData.getByCharacterId(pIdol.character_id).filter(idol=>idol.plan==pIdol.plan);
        DOM_set_character(element_sub1_character_select, dPIdols);
        DOM_set_character(element_sub2_character_select, dPIdols);
        DOM_set_character(element_sub3_character_select, dPIdols);
        DOM_set_character(element_sub4_character_select, dPIdols);
        ...
        element_sub1_character_select.dispatchEvent(new Event('change'));
        element_sub2_character_select.dispatchEvent(new Event('change'));
        element_sub3_character_select.dispatchEvent(new Event('change'));
        element_sub4_character_select.dispatchEvent(new Event('change'));
    }, false);

    element_sub1_character_select.addEventListener('change', (e) => {
        const id = Number(e.target.value);
        const pIdol = PIdolData.getById(id);
        DOM_set_uniqueSkillCard('sub1', pIdol.unique_skillCard_id);
    }, false);
    element_sub2_character_select.addEventListener('change', (e) => {
        const id = Number(e.target.value);
        const pIdol = PIdolData.getById(id);
        DOM_set_uniqueSkillCard('sub2', pIdol.unique_skillCard_id);
    }, false);
    element_sub3_character_select.addEventListener('change', (e) => {
        const id = Number(e.target.value);
        const pIdol = PIdolData.getById(id);
        DOM_set_uniqueSkillCard('sub3', pIdol.unique_skillCard_id);
    }, false);
    element_sub4_character_select.addEventListener('change', (e) => {
        const id = Number(e.target.value);
        const pIdol = PIdolData.getById(id);
        DOM_set_uniqueSkillCard('sub4', pIdol.unique_skillCard_id);
    }, false);
    ...
    const element_contest_round_select = document.getElementById('contest-round-select');
    ...
    element_contest_stage_select.addEventListener('change', (e) => {
        ...
        const stageEffectsPItemIds = ContestData.getById(id).stages[idx].stageEffectsPItemIds;
        const fragment = document.createDocumentFragment();
        for(let i=stageEffectsPItemIds.length+1;i>0;i--){
            const option = document.createElement('option');
            option.innerHTML = `${i}轮`;
            option.value = i;
            fragment.appendChild(option);
        }
        DOM_delete_allChildren(element_contest_round_select);
        element_contest_round_select.appendChild(fragment);
        element_contest_round_select.dispatchEvent(new Event('change'));
    });
    ...
    function loadOptionsFromSearchParams() {
        ...
        if (contestStage) {
            ...
            element_contest_stage_select.value = contestStage.split(":")[0]+":"+contestStage.split(":")[1];
            element_contest_stage_select.dispatchEvent(new Event("change"));
            element_contest_round_select.value = contestStage.split(":")[2];
        }
        if (pIdol) {
            const [mainIdolId, sub1IdolId, sub2IdolId, sub3IdolId, sub4IdolId] = pIdol.split(":");
            element_main_character_select.value = mainIdolId;
            element_main_character_select.dispatchEvent(new Event("change"));
            element_sub1_character_select.value = sub1IdolId;
            element_sub1_character_select.dispatchEvent(new Event("change"));
            element_sub2_character_select.value = sub2IdolId;
            element_sub2_character_select.dispatchEvent(new Event("change"));
            element_sub3_character_select.value = sub3IdolId;
            element_sub3_character_select.dispatchEvent(new Event("change"));
            element_sub4_character_select.value = sub4IdolId;
            element_sub4_character_select.dispatchEvent(new Event("change"));
        }
        if (status) {
        ...
        if (cards) {
            ...
            element_sub1_cards.forEach((elem, idx) => {
                elem.value = 2 * Math.floor(cardIds[1][idx] / 2);
                elem.parentNode.getElementsByClassName("checkbox")[0].checked =
                    cardIds[1][idx] % 2 > 0;
                elem.dispatchEvent(new Event("change"));
            });
            element_sub2_cards.forEach((elem, idx) => {
                elem.value = 2 * Math.floor(cardIds[2][idx] / 2);
                elem.parentNode.getElementsByClassName("checkbox")[0].checked =
                    cardIds[2][idx] % 2 > 0;
                elem.dispatchEvent(new Event("change"));
            });
            element_sub3_cards.forEach((elem, idx) => {
                elem.value = 2 * Math.floor(cardIds[3][idx] / 2);
                elem.parentNode.getElementsByClassName("checkbox")[0].checked =
                    cardIds[3][idx] % 2 > 0;
                elem.dispatchEvent(new Event("change"));
            });
            element_sub4_cards.forEach((elem, idx) => {
                elem.value = 2 * Math.floor(cardIds[4][idx] / 2);
                elem.parentNode.getElementsByClassName("checkbox")[0].checked =
                    cardIds[4][idx] % 2 > 0;
                elem.dispatchEvent(new Event("change"));
            });
    ...
    function saveOptiostoSearchParams() {
        ...
        +`cards=${(()=>{
            ...
            element_sub1_cards.forEach((elem, idx) => {
                ...
                j+=i
                if(idx!=element_sub1_cards.length-1){
                    j+=':'
                }
            })
            return j
            ...
            element_sub2_cards.forEach((elem, idx) => {
                ...
                j+=i
                if(idx!=element_sub2_cards.length-1){
                    j+=':'
                }
            })
            return j
            ...
            element_sub3_cards.forEach((elem, idx) => {
                ...
                j+=i
                if(idx!=element_sub3_cards.length-1){
                    j+=':'
                }
            })
            return j
            ...
            element_sub4_cards.forEach((elem, idx) => {
                ...
                j+=i
                if(idx!=element_sub4_cards.length-1){
                    j+=':'
                }
            })
            return j
    ...
    element_run_button.addEventListener('click', async() => {
        ...
        const pItemIds_tmp2 = Array.from(new Set(pItemIds_tmp));
        ...
        // 重複を削除する機構がないため、仮削除
        const pidol_card_ids = [main_cards_id[0], sub1_cards_id[0], sub2_cards_id[0], sub3_cards_id[0], sub4_cards_id[0]];
        const pidol_card_ids_set = Array.from(new Set(pidol_card_ids));

        const skillCardIds = main_cards_id.slice(1).concat(sub1_cards_id.slice(1)).concat(sub2_cards_id.slice(1)).concat(sub3_cards_id.slice(1)).concat(sub4_cards_id.slice(1)).concat(pidol_card_ids_set).filter(id=>id&&id!='-1');
        ...
        const roundId = element_contest_round_select.value;
        const contestDetail = ContestData.getById(contestId);
        const contestStage  = contestDetail.stages[stageId];

        const pItemIds = contestStage.stageEffectsPItemIds.slice(0, roundId-1).concat(pItemIds_tmp2);
        ...
        const run_data = {
            ...
            criteria: contestStage.criteria,
            turnRank: contestStage.rank,
