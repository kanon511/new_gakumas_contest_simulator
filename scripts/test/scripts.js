...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function DOM_set_useSkillCards (skillCardIds) {
        const element = document.getElementById("use_card_list")
        let skillCards = [];
        skillCardIds.forEach(id => {
    ...
    function saveOptiostoSearchParams() {
        let str='https://kanon511.github.io/new_gakumas_contest_simulator/test.html?'
    ...
    element_run_button.addEventListener('click', async() => {
        ...
        switch (pIdol.plan) {
            case 'sense':
                default_skillCardIds = [1010010, 1011010, 1011020, 1021010, 1021020];
                break;
            case 'logic':
                default_skillCardIds = [1012010, 1012020, 1020010, 1022010, 1022020];
                break;
        }
        ...
        const container =  document.getElementById(`contest-log-min`);
        DOM_delete_allChildren(container);

        DOM_set_useSkillCards(skillCardIds);

        const result = run(run_data,container);

        //container.appendChild(parseSimulationLog(result));
